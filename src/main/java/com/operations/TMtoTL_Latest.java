...
public class TMtoTL_Latest {

	public static void main(String[] args) {
		
		String TM_SheetName = "TM";
		String TL_SheetName = "TM";
		
		System.out.println(System.getProperty("user.dir"));
		//Xls_Reader reader = new Xls_Reader(System.getProperty("user.dir")+"/src/main/java/com/excel/MISData.xlsx");
	
		Xls_Reader TM_reader = new Xls_Reader(System.getProperty("user.dir")+"/src/main/java/com/excel/TM_Tracker_Latest.xlsx");
		int TM_rowCount = TM_reader.getRowCount(TM_SheetName);
		
		Xls_Reader TL_reader = new Xls_Reader(System.getProperty("user.dir")+"/src/main/java/com/excel/TL_Tracker_Latest.xlsx");
		int TL_rowCount = TL_reader.getRowCount(TL_SheetName);
		System.out.println("123");

		for (int rowNum = 3; rowNum <=TM_rowCount; rowNum++) {
			...
			String pending_yesterday = TM_reader.getCellData(TM_SheetName, "Pending from yesterday", rowNum);
			String today_resolved = TM_reader.getCellData(TM_SheetName, "Resolved today's ticket", rowNum);
			String pending_resolved = TM_reader.getCellData(TM_SheetName, "Resolved pending tickets", rowNum);
			String open_pending = TM_reader.getCellData(TM_SheetName, "Total pending", rowNum);
			...
			System.out.println(date+""+shift+" "+name+" "+ticket_no+" "+pending_yesterday+" "+""+today_resolved+""+pending_resolved+" "+open_pending+" "+
			  ...
			  TL_reader.setCellData(TL_SheetName, "Pending from yesterday", rowNum, pending_yesterday);		  
			  TL_reader.setCellData(TL_SheetName, "Resolved today's ticket", rowNum, today_resolved);
			  TL_reader.setCellData(TL_SheetName, "Resolved pending tickets", rowNum, pending_resolved);
